 ...
 * @subpackage BuddyBar Widget/includes
 */

class BuddyBar_Widget_Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'buddybar_widget_pre_sanitize', $sanitized );

		switch ( $this->type ) {

			case 'color'			:
			case 'radio'			:
			case 'select'			: $sanitized = $this->sanitize_unknown( $this->data ); break;
			...
			default 				: $sanitized = $this->sanitize_unknown( $this->data ); break;
		 ...
		 */
		do_action( 'buddybar_widget_post_sanitize', $sanitized );
	 ...
	 */
	private function sanitize_unknown( $input ) {
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'buddybar-widget' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'buddybar-widget' ) );
